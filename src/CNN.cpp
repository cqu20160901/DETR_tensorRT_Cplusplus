...
void CNN::EngineInference(cv::Mat &SrcImage, void **Buffers, const std::vector<int64_t> &BufferSize, cudaStream_t Stream)
{
    ...
    static DETR Postprocess;
    int ret = Postprocess.GetDETRDetectResult(ModelOutput, DetectiontRects_);
...
std::vector<float> CNN::PrepareImage(cv::Mat &SrcImage)
{
    ...
    // to float32
    rsz_img.convertTo(rsz_img, CV_32F);

    // Split the image into separate color channels
    std::vector<cv::Mat> channels;
    cv::split(rsz_img, channels);

    // Subtract fixed mean and divide by fixed variance for each pixel in each channel
    float mean[3] = {0.406, 0.456, 0.485};
    float variance[3] = {0.225, 0.224, 0.229};
    for (int i = 0; i < channels.size(); ++i)
    {
        channels[i] = (channels[i] / 255.0 - mean[i]) / variance[i];
    }

    // Merge the channels back into the image
    cv::merge(channels, rsz_img);
